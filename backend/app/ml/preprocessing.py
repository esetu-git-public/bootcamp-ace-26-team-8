...
these engineered columns *in addition to* the base numeric/categorical
...
# ✅ Updated expected model columns
...
# ✅ New helper for outlier flags
def _add_outlier_flags(df: pd.DataFrame) -> pd.DataFrame:
    """
    Adds the outlier indicator columns expected by the trained model.
    These columns were present during training and are required by the
    ColumnTransformer. Since inference only contains one record and the
    original IQR statistics are unavailable, we default these flags to 0.
    """
    df = df.copy()
    df["Income_was_outlier"] = 0
    df["LoanAmount_was_outlier"] = 0
    df["InterestRate_was_outlier"] = 0
    df["DTIRatio_was_outlier"] = 0
    return df
...
def build_feature_frame(payload: RequestLike) -> pd.DataFrame:
    ...
    return df.reindex(columns=EXPECTED_MODEL_COLUMNS)
